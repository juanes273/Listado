...
class ListadoViewModel : ViewModel() {
    ...
    fun createList(listName: String?) {
        ...
        FirebaseFirestore.getInstance().collection("lists")
            ...
            .addOnSuccessListener {
                Log.d("listado", "Lita Creada ${it.id}")
                Log.d("listado", "Lita Creada $listName")
            ...
            viewModelScope.launch(Dispatchers.IO) {
                try {
                    ...
                    if (!querySnapshot.isEmpty) {
                        ...
                        Log.d("ListadoViewModel", "Lista eliminada: $listName para el usuario actual con ID: $userId")
                    } else {
                        Log.d("ListadoViewModel", "No se encontró la lista $listName para el usuario actual con ID: $userId")
                    }
                } catch (e: Exception) {
                    // Manejar errores
                    Log.e("ListadoViewModel", "Error al eliminar la lista $listName para el usuario actual con ID: $userId", e)
                }
            }
        }
    }
    ...
    fun fetchObjectList(list: String) {
        if(list == "Selecciona lista"){
            _objectList.value = emptyList()
        }else{
            val userId = auth.currentUser?.uid
            viewModelScope.launch(Dispatchers.IO) {
                if (userId != null) {
    ...
    fun addObjectToList(newObject: String, list: String) {
        val userId = auth.currentUser?.uid
        if (userId != null) {
            viewModelScope.launch(Dispatchers.IO) {
                try {
    ...
    fun deleteObjectFromList(objectToDelete: String, list: String) {
        val userId = auth.currentUser?.uid
        if (userId != null) {
            viewModelScope.launch(Dispatchers.IO) {
                try {
                    ...
                    if (!querySnapshot.isEmpty) {
                        val document: DocumentSnapshot? = querySnapshot.documents[0]
                        if (document != null) {
                            // Obtén la lista actual de objetos
                            val currentObjects = document.get("objects") as? MutableList<String>

                            // Elimina el objeto de la lista
                            currentObjects?.remove(objectToDelete)
                            ...
                            Log.d("ListadoViewModel", "Objeto eliminado exitosamente de la base de datos: $objectToDelete")
                        }
                    } else {
                        Log.e("ListadoViewModel", "No se encontraron documentos para el usuario con ID: $userId")
                    }
                } catch (e: Exception) {
                    Log.e("ListadoViewModel", "Error al eliminar objeto de la base de datos", e)
                }
            }
        } else {
            Log.e("ListadoViewModel", "El usuario actual es nulo. Asegúrate de estar autenticado.")
